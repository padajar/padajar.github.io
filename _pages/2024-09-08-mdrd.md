...
- (a must) you know how student-proposing deferred acceptance works. in the next section, i do describe it a little — but i personally don’t think it’s fleshed out enough for someone who doesn’t know DA to understand deeply.[^0]
...
1. note that each school will have exactly one *marginal* priority group, which we’ll call $$\rho_s\in\{0, 1, 2, \dots, K\}$$. students who have $\rho_{\theta s} < \rho_s$ never are assigned to any school they like less than $s$, and students with $\rho_{\theta s} > \rho_s$ never get assigned to $s$. given a cutoff $c_s$, this marginal priority group is simple to find: $\rho_s = \lfloor{c_s}\rfloor$. in essence, it’s the set of students that have *direct* risk at $s$. however, other students will also still have risk at $s$ — the students with $\rho_{\theta s} < \rho_s$; but their risk will be due to their risk of not being admitted to schools they like more than $s$.
   ...
   * $t_{is}=a$ if $\rho_{\theta s} < \rho_s$; these are students who are *always* seated at $s$ if they end up submitting an application to $s$ during DA.
   * $t_{is} = c$ if $\rho_s = \rho_s$; these are students who are sometimes (*conditionally*) seated at $s$ if they end up submitting an application to $s$ during DA.
   * $t_{is}=n$ if $\rho_{\theta s} > \rho_s$; these students are never seated at $s$ if they submit an application to $s$ during DA.

   another way to think about it, that can be helpful:

   * if $t_{is}=a$, then $i$ is always assigned to $s$ or better under every lottery draw.
   * if $t_{is} = n$, then $i$ is never assigned to $s$ under any lottery draw.
   * if $t_{is} = c$, then either $i$ is always assigned to some school they like better, or $i$ is only assigned to $s$ under *some* lottery draws (and not all).
...
2. the **frequency** calculation.[^20] recall that 

   $$p_s(\theta) = P(\text{rejected from all } s' \in B_{\theta s}) \cdot P(\text{accepted to }s \mid \text{rejected from all } s' \in B_{\theta s}))$$

   we start off the same as the formula way: take observed cutoffs, and use them to calculate $\mathrm{MID}$s. we use these to calculate the first term in the equation.

   the formula calculation also uses these same cutoffs to calculate the second term. but in the frequency approach, we do something different. instead, we *estimate* the second term. we look at all students who *actually* got rejected from all $s’ \in B_{\theta s}$, and find their offer rate at $s$. we use that as our estimated propensity score.
...
[^20]: i’m actually going to explain this in a completely different way than MDRD1 does. refer to p. 1392 and the table at the top of p. 1393 for the original explanation. the reason for my change is that the main text of MDRD doesn’t allow for multiple score types. to talk about this in generality, i find it helpful to think about it in my way.
